...
class helper{
     ...
     *
     */
    timeDown(timestamp, date = false)
    {
        let result = {};
        let now = Math.round(new Date / 1000);
        let second = Math.floor(timestamp - now);
        ...
        if (year > 0) {
            result.year = year;
            result.month = month - year * 12;
            result.day = day - year * 12 * 31;
            result.hour = hour - year * 12 * 31 * 24;
            result.minute = minute - year * 12 * 31 * 24 * 60;
            result.second = second - year * 12 * 31 * 24 * 60 * 60;
        } else if (month > 0) {
            result.month = month;
            result.day = day - month * 31;
            result.hour = hour - month * 31 * 24;
            result.minute = minute - month * 31 * 24 * 60;
            result.second = second - month * 31 * 24 * 60 * 60;
        } else if (day > 0) {
            result.day = day;
            result.hour = hour - day * 24;
            result.minute = minute - day * 24 * 60;
            result.second = second - day * 24 * 60 * 60;
        } else if (hour > 0) {
            result.hour = hour;
            result.minute = minute - hour * 60;
            result.second = second - hour * 60 * 60;
        } else if (minute > 0) {
            result.minute = minute;
            result.second = second - minute * 60;
        } else if (second > 0) {
        ...
        if (date) {
            result.date = this.timeToDate(timestamp);
        }
        return result;
    }
